# 👋 Hi, I’m Muhammad Hammad Sohail

**Lead Software Engineer | Founder & CTO | Full-Stack, SaaS, Ecommerce, IoT & Real-Time Systems**

I build scalable SaaS platforms, ecommerce systems, real-time analytics pipelines, and cloud-native applications using **Node.js, Next.js, TypeScript, Go, NestJS, AWS, Docker, MongoDB, PostgreSQL, Redis, and InfluxDB**.

---

## 🚀 What I’m Working On in 2026

### 🛒 Eekaam / CloudyLeap — SaaS Website Builder for SMBs
I’m building a Shopify/Wix-style SaaS platform that helps small and medium businesses launch websites and online stores quickly.

**Key areas:**
- Multi-tenant SaaS architecture
- Storefronts, admin panels, and templates
- Subscription-based business model
- Product/catalog management
- Scalable hosting architecture
- Ecommerce workflows and automation

---

## 💼 Current & Recent Work

### ⚙️ WiserMachines — IoT & Real-Time Analytics
Led software engineering for an industrial IoT analytics platform supporting **700+ devices**.

**Highlights:**
- Built high-throughput data pipelines handling **3,500+ writes every 5 seconds**
- Worked with **InfluxDB, Docker, MongoDB, AWS, React Native, Kotlin, and real-time dashboards**
- Built backend and notification services for high-volume workloads
- Reduced AWS infrastructure cost by **80%+**
- Improved system reliability, monitoring, and platform performance

---

### 🛍️ MultiH — Ecommerce & CRM
Worked on a custom ecommerce and CRM platform for an herbal products business.

**Highlights:**
- Migrated WordPress systems to **React / Next.js v14 with TypeScript**
- Built and maintained custom CRM for orders and customers
- Managed AWS services including **Lambda, SQS, and S3**
- Improved frontend performance and scalability

---

### 🔗 CandidLeap — Form Connector App
Built a Webflow form automation and integration app.

**Highlights:**
- Developed form management and automation features
- Implemented scalable payment flows
- Added passwordless login using magic links
- Built webhook-based customer integrations
- Improved platform reliability and error handling

---

### 🧠 CalmTalk — Mental Health SaaS
Contributed to frontend and backend development for a mental health SaaS platform.

**Highlights:**
- Built frontend features using **Next.js**
- Developed APIs with **Node.js and Strapi**
- Worked with **GraphQL**
- Improved website architecture and feature delivery

---

### 💬 Amaka Studio — Custom Chat System
Built a real-time chat system for client collaboration.

**Highlights:**
- Used **NestJS and Socket.IO**
- Designed multi-tenant workspace separation
- Built scalable real-time messaging features

---

### ☁️ TCP SaaS — TechCelerant Platform
Worked on a hosting and subscription-based SaaS platform.

**Highlights:**
- Built with **NestJS, Next.js, Docker, and microservices**
- Integrated subscription workflows
- Worked on WooCommerce and Shopify-related integrations
- Improved deployment and platform architecture

---

## 🛠️ Tech Stack

### Languages
![JavaScript](https://img.shields.io/badge/JavaScript-000?style=for-the-badge&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-000?style=for-the-badge&logo=typescript)
![Go](https://img.shields.io/badge/Go-000?style=for-the-badge&logo=go)
![Python](https://img.shields.io/badge/Python-000?style=for-the-badge&logo=python)

### Frontend
![Next.js](https://img.shields.io/badge/Next.js-000?style=for-the-badge&logo=nextdotjs)
![React](https://img.shields.io/badge/React-000?style=for-the-badge&logo=react)
![Tailwind CSS](https://img.shields.io/badge/TailwindCSS-000?style=for-the-badge&logo=tailwindcss)

### Backend
![Node.js](https://img.shields.io/badge/Node.js-000?style=for-the-badge&logo=nodedotjs)
![NestJS](https://img.shields.io/badge/NestJS-000?style=for-the-badge&logo=nestjs)
![Express.js](https://img.shields.io/badge/Express.js-000?style=for-the-badge&logo=express)
![Strapi](https://img.shields.io/badge/Strapi-000?style=for-the-badge&logo=strapi)

### Databases & Infrastructure
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-000?style=for-the-badge&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-000?style=for-the-badge&logo=mongodb)
![Redis](https://img.shields.io/badge/Redis-000?style=for-the-badge&logo=redis)
![InfluxDB](https://img.shields.io/badge/InfluxDB-000?style=for-the-badge&logo=influxdb)

### Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-000?style=for-the-badge&logo=amazonaws)
![Docker](https://img.shields.io/badge/Docker-000?style=for-the-badge&logo=docker)
![Linux](https://img.shields.io/badge/Linux-000?style=for-the-badge&logo=linux)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-000?style=for-the-badge&logo=githubactions)

---

## 🤝 I’m Open To Collaborate On

- SaaS platforms
- Ecommerce systems
- Real-time dashboards
- IoT analytics platforms
- Backend architecture
- Go-based services
- AWS optimization
- Multi-tenant applications

---

## 📫 Reach Me

**Email:** [hammad@cloudyleap.com](mailto:hammad@cloudyleap.com)
**Email:** [muhammadhammad848@gmail.com](mailto:muhammadhammad848@gmail.com)

---

## 📊 GitHub Stats

![Hammad's GitHub Stats](https://github-readme-stats.vercel.app/api?username=hammadsohail&show_icons=true&theme=dark)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=hammadsohail&layout=compact&theme=dark)

---

<!---
hammadsohail/hammadsohail is a ✨ special ✨ repository because its README.md appears on your GitHub profile.
--->
